...
public class OCTGNDirectory {
	...
	private static final Predicate<OCTGNEntry> IS_LATEST_FILTER = e -> e.isLatest();
	private URL feedUrl;
	...
	public OCTGNEntry getEntryForName(String gameName) throws IOException, URISyntaxException {
		Optional<OCTGNEntry> entry =
				getEntriesFromFeed()
				.stream()
				.filter(new NameFilter(gameName))
				.map(e -> new OCTGNEntry(e))
				.filter(IS_LATEST_FILTER)
				.findAny();
	...
	private class NameFilter implements Predicate<Entry> {
